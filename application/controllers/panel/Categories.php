...
class Categories extends CI_Controller
{
	...
	public function sub_index($page, $id)
	{
		if (checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			if (!$this->db->table_exists($page)) {
				$this->base_m->create_table($page);
			}
			// DEFAULT DATA
			$data = loadDefaultData();

			$data['cat'] = $this->back_m->get_one('categories', $id);
			$data['rows'] = $this->back_m->get_subcat($id);

			//$data['rows'] = $this->back_m->get_all($this->uri->segment(2));
			echo loadSubViewsBack($this->uri->segment(2), 'sub_index', $data);
		} else {
			redirect('panel');
		}
	}
	...
	public function sub_form($type, $cat, $id = '')
	{
		if (checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			// DEFAULT DATA
			$data = loadDefaultData();

			$data['cat'] = $this->back_m->get_one('categories', $cat);

			if ($id != '') {

				$data['value'] = $this->back_m->get_one('subcat', $id);
			}
			echo loadSubViewsBack($this->uri->segment(2), $type, $data);
		} else {
			redirect('panel');
